...
class IAF_DSF(BaseFlow):
    ...
    def __init__(self, dim, hid_dim, context_dim, num_layers,
        ...
        if type(dim) is int:
            self.mdl = iaf_modules.cMADE(
                    ...
                    num_ds_multiplier*(hid_dim//dim)*num_ds_layers, ##not entirely clear
                    activation, fixed_order)
    ...
    def reset_parameters(self):
        self.out_to_dsparams.weight.data.uniform_(-0.001, 0.001) ##RANDOMIZATION FOR DSF PARAMETERS
        self.out_to_dsparams.bias.data.uniform_(0.0, 0.0) #Why such a specific initialization for bias
    ...
    def forward(self, inputs):
        ...
        out, _ = self.mdl((x, context))
        if isinstance(self.mdl, iaf_modules.cMADE):
            out = out.permute(0,2,1) #permutations could be another source of randomization
